...
int main()
{
    //OPERÁTOROK TESZTELÉSE - INT TÖMBÖKET HASZNÁLOK
    Gen<int> x(7), x2(4), x5(6), xx;
    //ertekadasok - ezekkel a számokkal jobban látszik az operátorok mûködése, ezért itt nem használok fájlbeolvasást
    for(int i=0; i<7;i++)
    {
        x[i]=1;
    ...
    //MARADÉK SZÁMOKRA ÍRT FÜGGVÉNYEK MEGHÍVÁSA
    ...
    //FÁJLBEOLVASÁS - EZEKKEL FOGOM TESZTELNI A SHELLEKET - DOUBLE TÖMBÖKET HASZNÁLOK
    //elsõ tömb beolvasása
    ifstream f;
    string file;
    ...
    if (f.is_open()) //a fájl létezésének ellenõrzése
    {
        f>>meret;
        cout<<meret;
        Gen<double> x3(meret);
        while(f.good()) //addig olvas, amíg nincs error.
        {
            f>>x3[szam];
            szam++;         
        }
    ...
    if (f2.is_open()) //a fájl létezésének ellenõrzése
    {
        f2>>meret;
        Gen<double> x4(meret);
        while(f2.good()) //addig olvas, amíg nincs error.
        {
            f2>>x4[szam];
            szam++;         
        }
    ...
    //STRINGEKRE ÍRT FÜGGVÉNYEK TESZTELÉSE - ILLETVE AZ EDDIG NEM MEGHÍVOTT FÜGGVÉNYEKET IS ITT TESZTELEM
    //EZEKHEZ ELÉG 1 STRING TÖMB IS, SZINTÉN FÁJLBEOLVASÁSSAL OLDOM MEG
    ...
    if (f3.is_open()) //a fájl létezésének ellenõrzése
    {
        f3>>meret;
        Gen<char*> x6(meret);
        ...
        szam=0;
        while(f3.good()) //addig olvas, amíg nincs error.
        {
